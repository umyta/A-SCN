...
all_data_labels = [line.rstrip() for line in open('meta/all_data_label.txt')]
pred_data_label_filenames = [f.rstrip('\.npy') + '_pred.txt' for f in all_data_labels ]
...
for i in range(num_room):
    print(i)
    pred_filepath = os.path.join('all_pred_folder', pred_data_label_filenames[i])
    data_label = np.loadtxt(pred_filepath)
    pred_label = data_label[:,-1]
    gt_filepath = os.path.join('all_pred_folder', gt_label_filenames[i])
    gt_label = np.loadtxt(gt_filepath)
    print('pred file', pred_filepath, 'gt filepath', gt_filepath)

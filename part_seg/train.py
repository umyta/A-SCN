...
import provider
import ascn_part_seg as model

# DEFAULT SETTINGS
parser = argparse.ArgumentParser()
parser.add_argument('--gpu', type=int, default=0, help='GPU to use [default: GPU 0]')
parser.add_argument('--batch', type=int, default=16, help='Batch Size during training [default: 16]')
...
parser.add_argument('--num_heads', type=int, default=4, help='Number of heads for self attention net [default: 4]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:' + str(FLAGS.gpu)):
            ...
            labels_pred, seg_pred = model.get_model(pointclouds_ph, input_label_ph,
                                                    is_training=is_training_ph, bn_decay=bn_decay,
                                                    cat_num=NUM_CATEGORIES,
                                                    part_num=NUM_PART_CATS, weight_decay=FLAGS.wd)
        ...
        def train_one_epoch(train_file_idx, epoch_num):
            ...
            for i in range(num_train_file):
                ...
                ##subsampling
                ind_mask = np.array(range(cur_data.shape[1]))
                np.random.shuffle(ind_mask)
                ind_mask[ind_mask>=point_num] = -1
                cur_data = np.stack([cur_data[:,i,:] for i,num in enumerate(ind_mask) if num>=0],axis=1)
                cur_seg = np.stack([cur_seg[:,i] for i,num in enumerate(ind_mask) if num>=0],axis=1)
        ...
        for epoch in range(TRAINING_EPOCHES):
            ...
            total_seg_acc = eval_one_epoch(epoch)
            if total_seg_acc > best_acc:
                best_acc = total_seg_acc
                cp_filename = saver.save(sess, os.path.join(MODEL_STORAGE_PATH, 'best_model.ckpt'))
                printout(flog, 'Successfully store the best model, epoch %d, into '%epoch + cp_filename)
        ...
        cp_filename = saver.save(sess, os.path.join(MODEL_STORAGE_PATH, 'last_model.ckpt'))
        printout(flog, 'Successfully store the checkpoint model into ' + cp_filename)
        flog.close()

...
parser.add_argument('--model_path', default='train_results/trained_models/last_model.ckpt', help='Model checkpoint path')
parser.add_argument('--output_dir', default='test_results', help='Save directory')
...
ply_data_dir = os.path.join(BASE_DIR, 'data/PartAnnotation')
gpu_to_use = 0
...
test_file_list = os.path.join(BASE_DIR, 'data/testing_ply_file_list.txt')
...
def predict():
    ...
    with tf.device('/gpu:'+str(gpu_to_use)):
        ...
        # simple model
        pred, seg_pred = model.get_model(pointclouds_ph, input_label_ph,\
                    is_training=is_training_ph, bn_decay=None, cat_num=NUM_OBJ_CATS, \
                    part_num=NUM_PART_CATS, num_heads=1, weight_decay=None)
